...
public class RentACatUnitTest {
	...
	public void setUp() throws Exception {
		...
		// TODO: Fill in
		c2 = Cat.createInstance(InstanceType.MOCK, 1, "Old Deuteronomy");
		...
		// TODO: Fill in
		c3 = Cat.createInstance(InstanceType.MOCK, 1, "Mistoffelees");
		...
		doAnswer(invocation -> {
			...
			return null;
		}).when(c2).renameCat("Garfield");
	
		doAnswer(invocation -> {
			when(c2.getRented()).thenReturn(true);  // After rentCat is called, set rented to true
			return null;
		}).when(c2).rentCat();

		doAnswer(invocation -> {
			when(c2.getRented()).thenReturn(false);  // After returnCat is called, set rented to false
			return null;
		}).when(c2).returnCat();
